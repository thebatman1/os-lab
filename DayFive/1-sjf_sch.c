...
int main(void) {
	...
	int cur_time = 0, cur_burst_time = 999, cur_process = -1, done = 0, flag_changed = 0;
	while (done < n) {
		for (int i = 0; i < n; ++i) {
			if (cur_time >= p[i].arr_time && p[i].burst_time != 0) {
				if (p[i].burst_time < cur_burst_time) {
					cur_burst_time = p[i].burst_time;
					cur_process = i;
					flag_changed = 1;
				}
			}
